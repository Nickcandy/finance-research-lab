...
Task 4.1：全量事件目录与单事件按需分析（已完成）
Task 4.2：免费 A 股公司语义库与确定性产业链映射（已完成）
Task 4.3：纯个股行情识别与分析禁用（已完成）
Task 4.4：事件及个股利好利空与影响指数（已完成）
Task 4.5：Watchlist 风险预警与全市场研究候选 Top 10（已完成）
...
新增 `sync-company-profiles`，使用 BaoStock 批量行业和 AkShare 的巨潮公司概况、同花顺主营介绍、
东财主营构成，按默认每批 100 只逐步填充 `a_share_universe.csv`。原始结果按 provider 和股票代码
保存在本地缓存，支持 TTL、失败隔离、原子写入和断点续跑；同步过程不调用 LLM。

首版本地词典覆盖 AI/CPO、半导体与存储、机器人、锂电、创新药/CXO。候选优先新闻明确公司、
同产品节点、一跳上下游和多跳关系；行业只用于上下文，不单独证明相关性。LLM 提出的股票如果没有
新闻直接提及或本地产品/产业链证据，保持 `unverified`，不得请求公司证据工具。东财概念板块成分
当前不稳定，继续保留为非阻塞 TODO。

### Task 4.3–4.5（已完成）

纯个股涨跌、涨跌停、成交额和市值播报继续保留在全量事件目录，但标记为 `not_applicable`，不进入
核心 Top 5，也不能触发单事件分析；板块、指数、商品以及包含公告、调查、业绩、订单或政策原因的事件
仍保留研究资格。

`DailyRadarSnapshot v2.1` 为事件和候选增加利好、利空、多空分化、中性、待判断方向，以及透明计算的
`-100..100` 影响指数和独立置信度。日报同时生成 Watchlist 中高强度风险预警，以及全市场已验证正向
研究候选 Top 10。两者只表达事件暴露和研究优先级，不是收益预测、买卖建议、目标价或仓位建议。
...
1. `DailyRadarSnapshot v2` 同时保存 Top 5 完整分析和全部聚类摘要，完整输入另存为原子 event catalog。
2. 新增 `/events` 与 `/events/:id`，支持搜索、分批加载、聚类成员查看和单事件 Markdown 报告。
3. 单事件分析使用后台任务且同一时间只运行一个；成功与失败结果均按 `run_id + event_id` 保存。
4. 连续行情子句规则合并同标的重复播报，同时保留不同公司、方向、期次和型号保护。
5. 候选证券代码按 universe 的唯一公司名纠正；未解析候选不再调用财报和行情 provider。

### TODO（本轮不做）

- 纯行情涨跌原因归因；后续从价格异动反查公告、新闻和行业事件。
- point-in-time 信号账本、1/5/20 日结果标签和 walk-forward 样本外回测。
- 收益概率、买卖时机、目标价和仓位；未完成回测前不进入产品输出。
